...
function furkotTiles(options) {
  ...
  if (options.input === 'png') {
    ...
    else if (options.input === 'geojson') {
      options.contentType = 'application/json';
    }
    else if (options.input === 'pbf') {
      options.filter.push(require('./filter/pbf'));
    }
    if (options.output === 'svg') {
      options.filter.push(require('./filter/layers'));
      options.render = require('./render/svg');
    }
    else if (options.output === 'canvas') {
      options.filter.push(require('./filter/layers'));
      options.filter.push(require('./filter/flatten-geojson'));
      options.render = require('./render/canvas');
    }
    else if (options.output === 'webgl') {
